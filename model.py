...
from Database import equipos, Wis, Rina, Bift, Eit, Vf, ELin, EVin, Lsf, puntosvalidos, Nijn, Aif, Dif, puntos as puntos1, RPfg, largo, RP, resultadosPosibles, Rin, puntosInicio, VGig, listasuprema, dictPosiblesG, Tigf
...
def modelo(partidosPre):
    ...
    zz_ig = m.addVars(listasuprema, vtype=GRB.BINARY, name="Elige patron")
    ...
    m.addConstrs((quicksum(zz_ig[equipo, numerog] for numerog in dictPosiblesG[equipo]) == 1 for equipo in equipos))

    #R10

    m.addConstrs((quicksum(x_nf[partido, fecha] * Rin[equipo][partido] for partido in partidos
        if (ELin[equipo][partido] + EVin[equipo][partido]) == 1 and partido in Rin[equipo].keys()) == quicksum(zz_ig[equipo, numerog] * RPfg[fecha][numerog]
        for numerog in dictPosiblesG[equipo]) for equipo in equipos for fecha in fechas), name="R3")

    #R11
    """
    m.addConstrs(p_itf[equipo, punto, fecha] >= quicksum(zz_ig[equipo, numerog] for numerog in dictPosiblesG[equipo] if punto == puntosInicio[equipo] + Tigf[numerog][fecha]
                and VGig[equipo][numerog]) for equipo in equipos for fecha in fechasz
                 for punto in puntos1[equipo][fecha])
    
    m.addConstrs(p_itf[equipo,  puntosInicio[equipo] + Tigf[numerog][fecha], fecha] >= zz_ig[equipo, numerog]
                for equipo in equipos for fecha in fechas for numerog in dictPosiblesG[equipo])
    """
    m.addConstrs(p_itf[equipo, puntosInicio[equipo] + Tigf[numerog][fecha], fecha] >= zz_ig[equipo, numerog]
                 for equipo in equipos for fecha in fechas for numerog in dictPosiblesG[equipo])

    # R11
    m.addConstrs((p_itf[equipo, punto, fecha] == 0 for equipo in equipos for fecha in fechas for punto in puntos1[equipo][fecha]
                  if Bift[equipo][fecha][punto] == 0), name="R11")
    ...
    m.addConstrs((s_ft[fecha, punto] <= 1 - p_itf[equipo, punto2, fecha] for punto in puntos for punto2 in puntos
                  for equipo in equipos for fecha in fechas if punto < punto2 and punto2 in puntos1[equipo][fecha]), name="R134")
    ...
    m.addConstrs((a_if[equipo, fecha] <= (1 - p_itf[equipo, punto, fecha - 1]) + z_ft[fecha, punto]
                  for equipo in equipos for punto in puntos
                  for fecha in fechas if (fecha >= 17) and punto in puntos1[equipo][fecha - 1]), name="R12")
    ...
    m.addConstrs((l_ft[fecha, punto] <= 1 - p_itf[equipo, punto2, fecha] for punto in puntos for punto2 in puntos
                  for equipo in equipos for fecha in fechas if punto2 < punto and punto2 in puntos1[equipo][fecha]), name="R134")
